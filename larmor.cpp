//void plott()
{
     ...
     double x;
     TF1 *f1 = new TF1("f1", "[0]*(TMath::Exp(-x/[1])*(1+[2]*TMath::Cos([3]*x+[4]))+TMath::Exp(-(x-[5])/[1])*(1+[2]*TMath::Cos([3]*(x-[5])+[4])))+[6]", 2000, 10000); //フィッティングする為の関数の定義。[]で囲んである部分がフィットで変化させるパラメーター。関数を使うときはTMath::expのようにする。TMath::Gaus(Double_t x,Double_t mean,Double_t sigma)
     f1->SetNpx(50000);                                                                                                                                               //フィットした関数をプロットする細かさ

     f1->SetParameters(1, 2196.9811, 0.5, 0.0135, 6.18, 598, 0); //パラメータの初期値を決める

     f1->SetParNames("C_1", "#tau[ns]", "C_2", "#omega[rad/ns]", "#delta[rad]", "#Deltat[ns]", "C_3"); //パラメータの名前を決める
     h1->Fit("f1", "", "", 2000, 10000);                                                               //xが0.000000001から10の範囲についてフィット
